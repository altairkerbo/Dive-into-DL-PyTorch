...
x = torch.randn_like(x, dtype=torch.float) # 返回与x相同形状的正态分布矩阵，指定新的数据类型
print(x)
```
输出：
```
...
|gather(input, dim, index)|根据index，在dim维度上选取数据，index的size须与input一样，返回的size也与input一样|
...
**这两个函数所产生的的`Tensor`和NumPy中的数组不共享相同的内存，但改变其中一个时另一个也会改变！！！**
> 还有一个常用的将NumPy中的array转换成`Tensor`的方法就是`torch.tensor()`, 需要注意的是，此方法总是会进行数据拷贝，返回的`Tensor`和原来的数据不共享内存，同时也不共享数据，即改变其中一个，另一个不会随之改变。
